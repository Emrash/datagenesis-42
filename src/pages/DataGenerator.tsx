...
const DataGenerator: React.FC = () => {
  ...
  const checkBackendHealth = async () => {
    try {
      ...
      if (health.healthy) {
        ...
        addRealtimeLog(`Backend connection error`);
  ...
  const addRealtimeLog = (message: string) => {
    ...
    setRealtimeLogs(prev => [...prev.slice(-9), logEntry]);
  ...
  useEffect(() => {
    if (lastMessage?.type === 'generation_update') {
      ...
      if (data.progress === 100) {
        ...
        addRealtimeLog(`${method} generation finished!`);
        toast.success(`🎉 ${method} generation completed!`);
      } else if (data.progress === -1) {
        setIsGenerating(false);
        addRealtimeLog(`Generation failed: ${data.message}`);
  ...
  const handleGenerate = async () => {
    ...
    addRealtimeLog('🚀 Starting data generation...');
    ...
    try {
      ...
      if (inputMethod === 'upload' && uploadedData) {
        ...
        addRealtimeLog(`📁 Using uploaded data: ${sourceData.length} records`);
      } else if (inputMethod === 'describe' && generatedSchema) {
        sourceData = generatedSchema.sample_data || [];
        schema = generatedSchema.schema || {};
        addRealtimeLog(`🧠 Using AI-generated schema: ${Object.keys(schema).length} fields`);
      }
      ...
      addRealtimeLog(`⚙️ Configuration: ${generationConfig.rowCount} rows, ${selectedDomain} domain`);
      ...
      const methodLabels: Record<string, string> = {
        'backend_ai': '🤖 Multi-Agent AI',
        'backend_local': '🔗 Backend AI',
        'enhanced_local_fallback': '🏠 Enhanced Local',
        'local_fallback': '🏠 Local Fallback',
        'multi_agent_ai': '🎯 Multi-Agent System'
      };
      
      const methodLabel = methodLabels[method] || '✨ AI Generation';
      const rowsGenerated = result.metadata?.rowsGenerated || result.data.length;
      const qualityScore = result.metadata?.qualityScore || 0;
      
      addRealtimeLog(`✅ Generation complete: ${rowsGenerated} rows, ${qualityScore}% quality`);
      toast.success(
        `${methodLabel} generation complete! ${rowsGenerated} high-quality rows generated.`, 
        { duration: 5000 }
      );
      setIsGenerating(false);
      ...
      addRealtimeLog(`❌ Generation failed: ${errorMessage}`);
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <div className={`flex items-center gap-2 px-3 py-1 rounded-full border text-sm ${
            ...
            geminiStatus === 'online' ? 'bg-purple-500/20 border-purple-500/30' : 'bg-yellow-500/20 border-yellow-500/30'
          }`}>
      ...
      {!backendHealthy && (
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-red-300 font-medium">Backend Connection Lost</p>
              <p className="text-red-200 text-sm">
                Unable to connect to AI backend. Using local fallback generation.
                {lastHealthCheck && ` Last attempted: ${lastHealthCheck.toLocaleTimeString()}`}
              </p>
            </div>
          </div>
        </motion.div>
      ...
      {backendHealthy && geminiStatus === 'online' && (
        <motion.div
